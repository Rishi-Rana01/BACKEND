...
const refreshAccessToken = asyncHandler(async(req,res)=>{
  ...
  try {
    const decodedToken=jwt.verify(incomingRefreshToken, process.env.REFRESH_TOCKEN_SECRET)
...
const updateAccountDetails = asyncHandler(async(req,res)=>{
  ...
  const user = await User.findByIdAndUpdate(
    ...
    return res.status(200).json(new ApiResponse, user, "Account details update successfully")
})
